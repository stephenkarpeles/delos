    ...
    generateInlineSVG () {
      ...
      request.onload = function () {
        if (request.status >= 200 && request.status < 400) {
          ...
          // reset width/height if overrides are present. Otherwise don't touch
          if (context.width) {
            inlinedSVG.removeAttribute('width')
            inlinedSVG.setAttribute('width', context.width)
          }
          if (context.height) {
            inlinedSVG.removeAttribute('height')
            inlinedSVG.setAttribute('height', context.height)
          }
